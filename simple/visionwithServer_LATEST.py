...
model.names[0]  = "RipeBlackberry"
model.names[1]  = "RipeStrawberry"
model.names[2]  = "UnripeBlackberry"
model.names[3]  = "UnripeStrawberry"
...
while not rospy.is_shutdown():
    ...
    results = pred[0]
    annotated_frame = pred[0].plot(line_width = 1, font_size = 4)
    if model.task == 'obb':
        iterate = results.obb
    else:
    ...
    contador = 0
    fruits   =  []
    class_ids = iterate.cls.int().tolist() if iterate.cls is not None else []
    names = results.names
    

    counts = dict( Counter([names[cid] for cid in class_ids]))
    print(counts)
    countsSend = counts
